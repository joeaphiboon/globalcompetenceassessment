...
def main():
    ...
    if st.session_state.stage == 'self_assessment':
        ...
        with tab1:
            ...
            st.progress(attitude_cq_score / max_attitude_score)
            ...
            max_case_study_score = len(case_studies) * 8
            fig, ax = plt.subplots()
            ax.bar(range(1, len(case_study_scores) + 1), case_study_scores)
            ax.set_xlabel('Case Study')
            ax.set_ylabel('Score')
            ax.set_title('Case Study Scores')
            ax.set_ylim(0, 8)
            st.pyplot(fig)
